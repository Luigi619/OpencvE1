...
class AsyncServiceHelper
{
            ...
            final LoaderCallbackInterface Callback)
    {
        ...
        if (AppContext.bindService(intent, helper.mServiceConnection.get(), Context.BIND_AUTO_CREATE))
        {
            return true;
        }
        else
        {
            AppContext.unbindService(helper.mServiceConnection.get());
    ...
    protected final ThreadLocal<ServiceConnection> mServiceConnection;

    {
        mServiceConnection = new ThreadLocal<ServiceConnection>() {
                ...
                if (null == mEngineService) {
                    ...
                    try {
                        if (mEngineService.getEngineVersion() < MINIMUM_ENGINE_VERSION) {
                            ...
                            Log.d(TAG, "Unbind from service");
                            mAppContext.unbindService((ServiceConnection) mServiceConnection);
                            Log.d(TAG, "Calling using callback");
                            ...
                            if (!mLibraryInstallationProgress) {
                                InstallCallbackInterface InstallQuery = new InstallCallbackInterface() {
                                    ...
                                    public void cancel() throws IOException {
                                        ...
                                        Log.d(TAG, "Unbind from service");
                                        mAppContext.unbindService(mServiceConnection.get());
                                        Log.d(TAG, "Calling using callback");
